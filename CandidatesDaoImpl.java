...
public class CandidatesDaoImpl implements CandidateDao {
	...
	private PreparedStatement pst5,pst6;
	...
	public CandidatesDaoImpl() throws SQLException {
		...
		/*
		 ...
		 create procedure incrementVoting(IN can_id int , IN user_id tinyint, OUT
		  ...
		  begin
		  declare vote int ; 
		  select votes into vote from candidates where id = can_id;
		 set vote = vote+1; 
		 update candidates set votes = vote where id = can_id;
		  update users set status = 1 where id = user_id; 
		  select status into state from users where id = user_id;
		  end &&
	...
	pst6 = connection.prepareStatement("insert into users values(default,?,?,?,?,?,?,?)");
	}
	...
	public String registerNewUser(User newUser) throws SQLException {
		pst6.setString(1, newUser.getFirstName());
		pst6.setString(2, newUser.getLastName());
		pst6.setString(3, newUser.getEmail());
		pst6.setString(4, newUser.getPassword());
		pst6.setDate(5, newUser.getDob());
		pst6.setBoolean(6, false);
		pst6.setString(7, "voter");
